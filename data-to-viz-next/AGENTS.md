...
- 应用场景图的颜色映射必须“可追溯且可验证”：必须明确 `color` 对应字段/函数、`scale.domain/range`、`legend` 标题与业务含义一致；对层级图（如 treemap/sunburst）优先使用显式函数映射（如 `color: (d) => d.data?.risk_band`），避免字段路径失效导致颜色回退默认值。提交前必须人工核对“颜色是否随数据分层变化”，不得仅以代码通过编译为准。
