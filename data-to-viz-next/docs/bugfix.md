...
### 现象
- `chart-map-hexbin` story 的“应用场景图”先后出现过空白、误显示为省级区划图、以及热点数据没有落到对应蜂窝中的问题。
...
### 根因
- 将用户要的“官方示例那种中国蜂窝轮廓”误实现成了“中国区划底图”，图形语义偏离。
- 一版修改中遗漏了 `chart.render()`，导致应用场景图直接空白。
- 热点层与底图层使用了不同的 `bin.hexagon` 网格参数；底图是 `binWidth: [2, 3]`，热点层却使用更细网格，导致热点没有映射到同一套蜂窝。
- 在尝试 `point + hexagon` 替代 `polygon` 时，忽略了 `bin.hexagon` 返回的是六边形顶点数组，而不是中心点。

### 修复方案
- 应用场景图回归官方示例语义：底层使用本地 `hexbin-china.json` 渲染中国蜂窝轮廓，热点层继续使用 `polygon` 六边形覆盖。
- 补回 `chart.render()`，恢复场景图实际渲染。
- 底图层与热点层统一使用同一套 `bin.hexagon` 网格参数（`binWidth: [2, 3]`），保证热点落在对应蜂窝中。
- 明确约束：若改为 `point + hexagon` 路径，必须先把顶点数组转换为中心点；若目标是复刻官方示例，则优先保持 `polygon` 链路。

### 验证要点
- 应用场景图能正常显示，不再空白。
- 底图为中国蜂窝轮廓，而不是省级行政区划面。
- 广州、深圳、成都的热点蜂窝与全国蜂窝底图严格对齐，不再出现“底图一套网格、热点另一套网格”的错位。
- 页面文案、图形语义与“蜂窝聚合”保持一致。
